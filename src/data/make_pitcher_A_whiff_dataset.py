...
# Load in pitcher A data
pitcher_A_data = pd.read_csv("../../data/processed/part2/pitcher_A_data.csv")

# Isolate PLATE_X, PLATE_Z, AND VERTICAL_APPROACH_ANGLE
pitcher_A_data = pitcher_A_data[pitcher_A_data["PITCH_TYPE_KEY"] == "FB"]
plate_x = pitcher_A_data.loc[:,pitcher_A_data.columns == "PLATE_X"]
plate_z = pitcher_A_data.loc[:,pitcher_A_data.columns == "PLATE_Z"]
vertical_approach_angle = pitcher_A_data.loc[:,pitcher_A_data.columns == "VERTICAL_APPROACH_ANGLE"]
...
print(df.head())
df.to_csv('../../data/processed/part2/X_predict_A.csv')